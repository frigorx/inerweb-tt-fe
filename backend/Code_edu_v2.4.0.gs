/**
 * ═══════════════════════════════════════════════════
 * inerWeb Édu — Google Apps Script Backend v2.4.0
 * Base v2.3.0 + Sprint 1 : Onglet Referentiels + getReferentiel API
...
// ── Configuration ──────────────────────────────
const CONFIG = {
  SPREADSHEET_ID: '1RTJ9jFhQtHgfXBUqMgj01wH-8MNGMGjDSjcFjkYjleA', // ← À REMPLACER !
...
function pushEvents(events) {
  ...
  const TYPES_AUTORISES = ['seance.validee','seance.invalidee','seance.annulee','seance.creee','competence.evaluee','import.ecole_directe'];
...
function getReferentiel(diplomeId) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (String(data[i][6]) !== 'TRUE' && data[i][6] !== true) continue; // actif uniquement
    competences.push({
      diplomeId: data[i][0],
      capaciteCode: data[i][1],
      capaciteLibelle: data[i][2],
      competenceCode: data[i][3],
      competenceLibelle: data[i][4],
      epreuveCode: data[i][5],
      actif: true
    });
  }
  return { competences };
...
/**
 * Peuplement des référentiels de compétences officiels.
 * Sources : Arrêté 2015 (CAP IFCA), Arrêté 2021 (Bac Pro MFER), Séminaire FDM TNE Versailles 2021.
 * À exécuter UNE FOIS après setupSpreadsheet().
 */
function populateReferentiels() {
  ...
  const rows = [
    // ── TNE — Seconde Famille des Métiers TNE (9 CC) ──
    ...
    // ── BAC PRO MFER — Arrêté 10 mars 2021 ──
    // ⚠️ Grain = compétence (pas de sous-compétences récupérées — PDF officiel inaccessible)
...
/**
 * Import emploi du temps depuis l'URL iCal École Directe
 * Appelé en POST : { action: 'importEcoleDirecte', icalUrl: '...', enseignant: 'ens-fh' }
 * 
 * Chaque collègue a sa propre URL :
 * https://api.ecoledirecte.com/v3/ical/P/{ID}/{TOKEN}.ics
 */
function importEcoleDirecte(icalUrl, enseignantId) {
  if (!icalUrl || !enseignantId) {
    return { error: 'URL iCal et enseignant requis' };
  }
  
  // Validation URL
  if (!icalUrl.includes('ecoledirecte.com') || !icalUrl.endsWith('.ics')) {
    return { error: 'URL invalide. Format attendu : https://api.ecoledirecte.com/v3/ical/P/.../xxx.ics' };
  }
  
  // Fetch iCal depuis École Directe (côté serveur = pas de CORS)
  let icalText;
  try {
    const resp = UrlFetchApp.fetch(icalUrl, { muteHttpExceptions: true });
    if (resp.getResponseCode() !== 200) {
      return { error: 'École Directe a répondu ' + resp.getResponseCode() + '. Vérifie ton URL.' };
    }
    icalText = resp.getContentText();
  } catch (e) {
    return { error: 'Impossible de joindre École Directe : ' + e.message };
  }
  
  if (!icalText.includes('BEGIN:VCALENDAR')) {
    return { error: 'Le contenu récupéré n\'est pas un calendrier iCal valide.' };
  }
  
  // Parse les événements
  const events = parseIcalEvents_(icalText);
  if (events.length === 0) {
    return { error: 'Aucun événement trouvé dans le calendrier.' };
  }
  
  // Convertir en séances
  const seances = [];
  const sheet = getOrCreateSheet(TABS.SEANCES);
  ...
  const data = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    existingIds.add(data[i][0]);
  }
  
  for (const evt of events) {
    const seanceId = 'ed-' + evt.uid;
    if (existingIds.has(seanceId)) continue; // Doublon → skip
    ...
    const seance = [
      seanceId,                                     // ID
      parsed.classe,                                // Classe
      dateParis,                                    // Date
      horaire,                                      // Horaire
      detectType_(parsed.classe, evt.summary),      // Type
      '',                                           // séquenceId (à remplir par le prof)
      evt.summary || parsed.classe,                 // séquenceNom (matière ÉD)
      buildContenu_(evt.summary, parsed, evt.location), // Contenu
      '',                                           // compétences (à remplir)
  ...
  if (seances.length === 0) {
    return { success: true, imported: 0, message: 'Toutes les séances étaient déjà importées.' };
  }
  ...
  try {
    lock.waitLock(10000);
    const lastRow = sheet.getLastRow();
    sheet.getRange(lastRow + 1, 1, seances.length, 12).setValues(seances);
  } finally {
    lock.releaseLock();
  }
  ...
  logSheet.appendRow([
    'evt-import-' + Date.now(),
    isoParis_(new Date()),
    'import.ecole_directe',
    enseignantId,
    icalUrl.substring(0, 60) + '...',
    JSON.stringify({ count: seances.length, classes: [...new Set(seances.map(s => s[1]))] }),
    'api'
  ]);
  ...
  for (const s of seances) {
    classes[s[1]] = (classes[s[1]] || 0) + 1;
  }
  
  return {
    success: true,
    imported: seances.length,
    total_events: events.length,
    duplicates: events.length - seances.length,
    classes: classes,
    periode: {
      debut: seances.length ? Utilities.formatDate(seances[0][2], 'Europe/Paris', 'yyyy-MM-dd') : '',
      fin: seances.length ? Utilities.formatDate(seances[seances.length-1][2], 'Europe/Paris', 'yyyy-MM-dd') : '',
    },
  };
}
...
function parseIcalEvents_(text) {
  ...
  for (let i = 1; i < blocks.length; i++) {
    ...
    // Unfold lignes (RFC 5545 : continuation par espace/tab en début de ligne)
    ...
    for (const line of unfolded.split(/\r?\n/)) {
      ...
      switch (key) {
        case 'UID': evt.uid = val; break;
        case 'DTSTART': evt.dtstart = val; break;
        case 'DTEND': evt.dtend = val; break;
        case 'SUMMARY': evt.summary = val; break;
        case 'DESCRIPTION': evt.description = val; break;
...
// ── Parseur description ÉD ───────────────────────
// Format : "PROF1 NOM., PROF2 NOM. - CLASSE"
// ou      "PROF NOM. - CLASSE"

function parseEDDescription_(desc) {
  if (!desc) return { profs: [], classe: '?' };
  
  const parts = desc.split(' - ');
  if (parts.length < 2) return { profs: [desc], classe: desc };
  
  const profsRaw = parts[0];
  const classe = parts.slice(1).join(' - ').trim();
  const profs = profsRaw.split(',').map(p => p.trim()).filter(Boolean);
...
function detectType_(classe, summary) {
  const cl = (classe || '').toUpperCase();
  const su = (summary || '').toUpperCase();
  
  if (cl.includes('CFA') || cl.includes('ETAM') || cl.includes('MPI')) return 'rassemblement';
  if (su.includes('CO-INTERVENTION')) return 'co-intervention';
  if (cl.includes('TP CVC') || cl.includes('TP SUP')) return 'complement';
  return 'hebdo';
}
...
function convertUTCtoParis_(dtstr) {
  // Format: 20260304T133000Z
  if (!dtstr) return new Date();
  const y = parseInt(dtstr.substr(0, 4));
  const m = parseInt(dtstr.substr(4, 2)) - 1;
  const d = parseInt(dtstr.substr(6, 2));
  const h = parseInt(dtstr.substr(9, 2));
  const min = parseInt(dtstr.substr(11, 2));
  return new Date(Date.UTC(y, m, d, h, min, 0));
}

function formatHoraireParis_(dtstart, dtend) {
  const start = convertUTCtoParis_(dtstart);
  const end = convertUTCtoParis_(dtend);
  const fmtH = (d) => Utilities.formatDate(d, 'Europe/Paris', 'HH:mm');
...
function buildContenu_(summary, parsed, location) {
  let parts = [];
  if (summary) parts.push(summary);
  if (parsed.profs && parsed.profs.length > 1) {
    const autres = parsed.profs.filter(p => !p.toUpperCase().includes('HENNINOT'));
    if (autres.length > 0) parts.push('Co-enseignement avec ' + autres.join(', '));
  }
  if (location) parts.push('Salle ' + location);
  return parts.join(' — ') || parsed.classe;
}
